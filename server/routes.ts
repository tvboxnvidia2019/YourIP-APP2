...
async function getIPInformation(ip: string, userAgent: string): Promise<IPInfo> {
  try {
    ...
    // Get IPv6 information if available (optional, don't let failures affect the main response)
    try {
      ...
      // IPv6 lookup is optional - silently continue without it
      // Only log in development mode to avoid console noise in production
      if (process.env.NODE_ENV === 'development') {
        console.debug('IPv6 lookup unavailable (not critical):', (error as Error).message);
      }
      // Set to undefined instead of null to match schema
      ipInfo.ipv6 = undefined;
    }
    ...
    // Return minimal info if everything fails - ensure app never breaks
    return {
      ip: ip,
      city: 'Unknown',
      region: 'Unknown',
